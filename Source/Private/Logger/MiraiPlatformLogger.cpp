...
MiraiPlatformLogger::MiraiPlatformLogger(const char* Identity)
{
	...
	_MiraiPlatformLoggerHandle = GMiraiSymbols->kotlin.root.net.mamoe.mirai.utils.PlatformLogger.PlatformLogger(Identity);
	if (_MiraiPlatformLoggerHandle.pinned == nullptr)
	{
...
MiraiPlatformLogger::~MiraiPlatformLogger()
{
	if (_MiraiPlatformLoggerHandle.pinned != nullptr)
	{
		GMiraiSymbols->DisposeStablePointer(_MiraiPlatformLoggerHandle.pinned);
		_MiraiPlatformLoggerHandle.pinned = nullptr;
	}
}
...
{
	if (_MiraiPlatformLoggerHandle.pinned == nullptr || Message == nullptr)
	{
		return;
	}
